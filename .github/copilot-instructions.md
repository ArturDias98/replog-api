...
- **replog-api**: Sync Lambda host. Hosts `/api/sync/*` and `/api/sync/health`. **Carries no auth** — trusts the `x-user-id` header injected by the gateway authorizer (`TrustedUserMiddleware`) and holds no JWT secret. References `replog-api-host`, `replog-application`, `replog-infrastructure`.
...
- **replog-api-authorizer**: API Gateway HTTP API REQUEST authorizer Lambda (plain class library). Validates the `access_token` cookie with the shared `AccessTokenValidator` and returns `userId` as authorizer context for `/api/sync/*`. References `replog-api-auth-core` only.
